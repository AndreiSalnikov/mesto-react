...
function PopupWithForm({name, id, title, onClose, isOpen, textOnButton, children, onSubmit,textLoad,isLoading}) {
  ...
  return (<Popup isOpen={isOpen} onClose={onClose} id={id}>
    <div className="popup__container">
      ...
      <form onSubmit={onSubmit} name={name} id="popupEditForm" className="popup__form" noValidate>
        {children}
        <button type="submit" id="submitEditButton" className="popup__save-button">{isLoading ? textLoad :textOnButton}</button>
      </form>
    </div>
  </Popup>);
}
